...
def url(site):
    try:
        if OKadminFinder.checkUrl(site, proxies):
            ...
            urls = tqdm(OKadminFinder.getUrls('http/okadminfinder3/LinkFile/adminpanellinks.txt'), bar_format="{l_bar}{bar}|{n_fmt}/{total_fmt}{postfix}")
        else:
            messenger.writeMessage('  Something wrong with url', 'red')
            urls = tqdm(OKadminFinder.getUrls('http/okadminfinder3/LinkFile/adminpanellinks.txt'), bar_format="{bar}")
            exit(SystemExit)
        ...
        for url in urls:
            ...
            if OKadminFinder.checkUrl(reqLink, proxies):
                ...
                messenger.writeMessage('\n' + '    {:<50}'.format('[✔] http://' + reqLink,) + '{:>30}'.format('Admin page found!\n'), 'bright')
...
def random_agent():
    useragent = "http/okadminfinder3/LinkFile/user-agent.txt"
...
def interactive():
    ...
    ┆''');
        ...
        # Get links for checking
        urls = OKadminFinder.getUrls('http/okadminfinder3/LinkFile/adminpanellinks.txt')
